...
if __name__ == "__main__":
    ...
    def get_result(query):
        ...
        input_queries, query_lengths = [preprocessed_query]*len(indexed_reply_set), [query_length]*len(indexed_reply_set)
        input_replies, reply_lengths = list(indexed_reply_set), list(reply_set_lengths)
        ...
        result = sorted([(reply, "{:.4f}".format(score[0]))
                         for reply, score in zip(reply_set, probs)], 
                        key=lambda x: x[1], reverse=True)[:10]

        return "{}\t{}".format(query, 
                               "\t".join(["\t".join(item) for item in result]))

    data_dir = "../reply_matching_model/data/crowdworks_queries.txt"
    target_dir = "../reply_matching_model/crowdworks/pinpong_queries.txt"
    with open(data_dir, "r") as f1, open(target_dir, "a") as f2:
        for i in tqdm(range()):
            line = f1.readline()
            query = line.strip()
            f2.write(get_result(query) + "\n")
