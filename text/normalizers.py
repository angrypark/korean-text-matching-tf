...
class MatchingModelNormalizer(BaseNormalizer):
    def __init__(self):
        self.patterns = {"month": re.compile("[0-9]{1,2}월"),
                         ...
                         "num": re.compile("\d+"),
                         'url': re.compile(
                             '(https?:\/\/|s?ftp:\/\/)?([\da-z\.-_?]+)\.([a-z]{2,6})([\/\w_\.-_?&%]*)*\/?'),
    ...
    def normalize(self, message, url=True, convert_hour=True, convert_month=True, repeat=True, special=True,
                  dots=True, mask_num=True, emoticon=True, double_space=True):
        ...
        if convert_hour:
            ...
            for hour in hour_patterns:
                if int(hour) < 13:
                    converted_hour = self.convert_hour[hour]
                    result = result.replace(hour, converted_hour)
                elif int(hour) < 25:
                    converted_hour = self.convert_hour[str(int(hour) - 12)]
        ...
        if convert_month:
            ...
            for month in month_patterns:
                if int(month) < 13:
                    converted_month = self.convert_month[month]
